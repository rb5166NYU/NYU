...
int process_page_access_fifo(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp) {
    ...
    if (*frame_cnt > 0) {
        ...
        (*table_cnt)++;  // Update the table count for a new entry
        return new_frame;
    }

    // Replace a page using FIFO
    int oldest_index = -1;
    for (int i = 0; i < TABLEMAX; i++) { // Loop through all entries
    ...
    page_table[oldest_index].is_valid = 0; // Mark it as invalid

    // Insert the new page
    page_table[oldest_index].frame_number = page_table[page_number].frame_number; // Reuse frame number
    page_table[page_number].frame_number = page_table[oldest_index].frame_number; // Set frame for new page
    page_table[page_number].is_valid = 1;
    page_table[page_number].arrival_timestamp = current_timestamp;
    page_table[page_number].last_access_timestamp = current_timestamp;
    page_table[page_number].reference_count = 1;
    return page_table[page_number].frame_number; // Return new frame
}
...
int process_page_access_lru(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp) {
    ...
    page_table[lru_index].is_valid = 0; // Mark it as invalid

    // Insert the new page
    page_table[page_number].frame_number = page_table[lru_index].frame_number; // Reuse frame number
    page_table[page_number].is_valid = 1;
    page_table[page_number].arrival_timestamp = current_timestamp;
    page_table[page_number].last_access_timestamp = current_timestamp;
    page_table[page_number].reference_count = 1;
...
int process_page_access_lfu(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp) {
    ...
    page_table[lfu_index].is_valid = 0; // Mark it as invalid

    // Insert the new page
    page_table[page_number].frame_number = page_table[lfu_index].frame_number; // Reuse frame number
    page_table[page_number].is_valid = 1;
    page_table[page_number].arrival_timestamp = current_timestamp;
    page_table[page_number].last_access_timestamp = current_timestamp;
    page_table[page_number].reference_count = 1;
