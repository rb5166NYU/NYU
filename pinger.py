...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        # unpack method is used to unpack the header into its different fields
        type, code, checksum, packetID, sequence = struct.unpack("bbHHh", icmpHeader)

        # Check if the packet ID matches the one sent in the ping request
        if packetID == ID:
            bytes = struct.calcsize("d")
            timeSent = struct.unpack("d", recPacket[28:28 + bytes])[0]
            rtt = (timeReceived - timeSent) * 1000
            ttl = ord(struct.unpack("c", recPacket[8:9])[0])
            return rtt, bytes, ttl
...
def sendOnePing(mySocket, destAddr, ID):
    ...
    def ping(host, timeout=1):
        ...
        vars = vars.append({'min': str(round(response['rtt'].min(), 2)), 'avg': str(round(response['rtt'].mean(), 2)),
                            ...
                            'stddev': str(round(response['rtt'].std(), 2))}, ignore_index=True)
